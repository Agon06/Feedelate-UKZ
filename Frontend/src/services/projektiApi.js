...
// Merr instruksionet/template-et për një lëndë (për student)
export const getStudentInstructions = async (studentId, lendaId) => {
  const response = await fetch(`${API_BASE_URL}/studentet/${studentId}/projekti/${lendaId}/instructions`);
  return handleResponse(response);
};

// Shkarko fajllat e lidhur me instruksionet
export const downloadInstructionFile = async (studentId, lendaId, fileName) => {
  const response = await fetch(`${API_BASE_URL}/studentet/${studentId}/projekti/${lendaId}/instructions/${fileName}/download`);
